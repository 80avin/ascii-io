...
class AsciiArtGenerator {
  ...
  private exposureValue: number = 1.0;
  private autoGenerate: boolean = false;
  private autoGenerateTimeout: number | null = null;
  ...
  private async generateCharacterIntensities(
    ...
    this.charIntensities = intensities.sort(
      (a, b) => a.intensity - b.intensity,
    );
    console.log(
      "Character intensities initialized for",
      this.currentCharacterSet,
      ":",
      this.charIntensities,
    );
  }

  private measureCharacterAspectRatio(): number {
    // Create temporary canvas to measure character dimensions
    const tempCanvas = document.createElement('canvas');
    const tempCtx = tempCanvas.getContext('2d')!;
    
    // Use the same font as output textarea
    const outputTextarea = document.getElementById('output') as HTMLTextAreaElement;
    const computedStyle = window.getComputedStyle(outputTextarea);
    ...
    // Measure a representative character (use 'M' which is typically square-ish)
    const metrics = tempCtx.measureText('M');
    const charWidth = metrics.width;
    
    // For height, we need to measure the actual font metrics
    // Use the font's actual height measurements
    const charHeight = metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent;
    
    // If actualBoundingBox is not available (older browsers), estimate
    const effectiveHeight = charHeight || fontSize * 0.8; // Fallback estimation
    
    const aspectRatio = charWidth / effectiveHeight;
    console.log(`Character aspect ratio: ${aspectRatio} (width: ${charWidth}, height: ${effectiveHeight})`);
    
    return aspectRatio;
  }

  private getCorrectedAspectRatio(originalAspectRatio: number): number {
    const charAspectRatio = this.measureCharacterAspectRatio();
    return originalAspectRatio * charAspectRatio;
  }
  ...
  private debouncedGenerate(): void {
    ...
    this.autoGenerateTimeout = window.setTimeout(() => {
  ...
  private setupEventListeners(): void {
    ...
    const lockAspectRatioCheckbox = document.getElementById(
      "lock-aspect-ratio",
    ) as HTMLInputElement;
    ...
    autoGenerateCheckbox.addEventListener('change', () => {
      this.autoGenerate = autoGenerateCheckbox.checked;
      if (this.autoGenerate && this.previewCanvas.width > 0) {
        this.debouncedGenerate();
      }
    });

    // Font size slider for output (if exists)
    ...
    if (fontSizeSlider && fontSizeValue && outputTextarea) {
      fontSizeSlider.addEventListener("input", () => {
        const fontSize = fontSizeSlider.value;
        fontSizeValue.textContent = `${fontSize}px`;
        outputTextarea.style.fontSize = `${fontSize}px`;
      });
    }

    // Exposure/contrast slider (if exists)
    ...
    if (exposureSlider && exposureValue) {
      exposureSlider.addEventListener("input", () => {
        this.exposureValue = parseFloat(exposureSlider.value);
        exposureValue.textContent = `${this.exposureValue.toFixed(1)}x`;
        if (this.autoGenerate) {
          this.debouncedGenerate();
        }
      });
  ...
  private drawTextPreview(
    ...
    // Show aspect ratio control for text as well
    this.showAspectRatioControl();
  ...
  private showAspectRatioControl(): void {
    const aspectRatioControl = document.getElementById(
  ...
  private updateAspectRatioConstrainedValues(): void {
    ...
    const correctedAspectRatio = this.getCorrectedAspectRatio(this.imageAspectRatio);
    const newRows = Math.round(currentColumns / correctedAspectRatio);
    ...
    if (clampedRows !== newRows) {
      const adjustedColumns = Math.round(clampedRows * correctedAspectRatio);
      columnsInput.value = Math.max(
        50,
        Math.min(1000, adjustedColumns),
      ).toString();
    }
  }
  ...
  private updateRowsFromColumns(): void {
    ...
    const correctedAspectRatio = this.getCorrectedAspectRatio(this.imageAspectRatio);
    const newRows = Math.round(columns / correctedAspectRatio);
    const clampedRows = Math.max(50, Math.min(1000, newRows));
  ...
  private updateColumnsFromRows(): void {
    ...
    const correctedAspectRatio = this.getCorrectedAspectRatio(this.imageAspectRatio);
    const newColumns = Math.round(rows * correctedAspectRatio);
  ...
  private async generateAsciiArt(): Promise<void> {
    ...
    if (this.charIntensities.length === 0) {
      outputTextarea.value =
        "Error: Character intensities not yet calculated. Please wait a moment and try again.";
      return;
    }
    ...
    try {
      ...
      if (this.previewCanvas.width === 0 || this.previewCanvas.height === 0) {
        throw new Error(
          "No preview image. Please upload an image or enter text first.",
        );
      }
  ...
  private async convertCanvasToAscii(
    targetCols: number,
    targetRows: number,
  ): Promise<string> {
    ...
    for (let row = 0; row < targetRows; row++) {
      for (let col = 0; col < targetCols; col++) {
        ...
        avgBrightness = this.applyExposure(avgBrightness);
  ...
  private applyExposure(brightness: number): number {
    // Apply exposure curve: brightness^(1/exposure)
    // Lower exposure values (< 1.0) brighten the image and reveal more detail in darker areas
    // Higher exposure values (> 1.0) darken the image and increase contrast
    const normalized = brightness / 255;
    const adjusted = Math.pow(normalized, 1 / this.exposureValue);
